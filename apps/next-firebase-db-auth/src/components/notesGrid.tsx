...
export default function NotesGrid(props: { notesArray: NoteI[] }) {
    ...
    const styles = { noteContainer: `flex flex-col h-min rounded-lg col-span-1 row-span-${rowSpanAmount} items-center justify-between p-1 md:place-self-center md:w-full`, lastOddNoteContainer: 'last:col-span-2' }
    ...
    return (
        <div className="w-full h-min grid grid-cols-2 md:grid-cols-3 grid-flow-dense items-stretch gap-2">
            ...
            </div> */}
            {
                notes.map((note, i) => {
                    return (
                        <div key={i} className={`${styles.noteContainer} ${colors(i)} ${i % 2 == 0 ? styles.lastOddNoteContainer : ``}`}>
